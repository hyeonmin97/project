...
double AcX, AcY, AcZ, Tmp, GyX, GyY, GyZ; // 가속도(Acceleration)와 자이로(Gyro)
...
void initSensor()
{
    ...
    wiringPiI2CWriteReg8(fd, 0x6B, 0); //disable sleep mode
}
...
void getData()
{
    ...
    AcZ = read_raw_data(ACCEL_XOUT_H);
    GyX = read_raw_data(GYRO_XOUT_H);
    GyY = read_raw_data(GYRO_XOUT_H);
    GyZ = read_raw_data(GYRO_XOUT_H);
}
